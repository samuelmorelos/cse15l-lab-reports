...
In this example, I wanted to use star again to see if `grep -c` could search the entire directory and see what the output would look like. I did not include all of the output, but it consisted of every file in the "biomed" directory and a number on the right side that signifies the number of lines that contain the pattern within that file. Since "R26R" only appears in one file, it shows "17" appended to the correct file, while the others all have a "0" attached.
